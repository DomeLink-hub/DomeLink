...
export function suggestPlanIdFromPlotSize(plotSize?: number | null): string {
  const sqft = plotSize && plotSize > 0 ? plotSize : 3000;
  if (sqft <= 1500) return "basic";
  if (sqft <= 3000) return "standard";
  if (sqft <= 5000) return "premium";
  if (sqft <= 10000) return "pro-premium";
  if (sqft <= 20000) return "premium-class-a";
  if (sqft <= 50000) return "premium-class-a-plus";
